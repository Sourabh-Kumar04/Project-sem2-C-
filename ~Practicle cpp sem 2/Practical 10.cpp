...
int main () {
    float s1, s2, s3;
    cout << "Enter the side of the triangles: " ;
    cin >> s1 >> s2 >> s3;
    try {
        Triangle t(s1, s2, s3);
        if (sqrt(s1*s1 + s2*s2) == s3 || sqrt(s3*s3 + s2*s2) == s1 || sqrt(s1*s1 + s3*s3) == s2) {
            cout << "The triangle is a right angled triangle" << endl;
            cout << "Area of the triangle by right angled triangled formula: " << t.area() << endl;
        }
        else{
