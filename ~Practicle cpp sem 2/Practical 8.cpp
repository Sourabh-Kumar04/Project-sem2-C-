...
class Matrix {
    ...
    public:
        ...
        Matrix product(Matrix m) {
            if (cols != m.rows) {
                throw "Matrix are not compatible for multiplication";
            }
            ...
            for (int i = 0; i < rows; i++) {
                for (int j = 0; j < m.cols; j++) {
                    ...
                    for (int k = 0; k < cols; k++) {
                        result.matrix[i][j] += matrix[i][j] * m.matrix[k][j];
                    }
                }
            }
            return result;
...
int main () {
    ...
    int choice;
    cout << "Enter the operation to perforn: " << endl;
    cout << "1. Sum" << endl;
    cout << "2. Product" << endl;
    ...
    switch (choice) {
        ...
        case 2: {
            int rows2, cols2;
            cout << "Enter the number of rows and columns of thesecond matrix: " ;
            cin >> rows2 >> cols2;
